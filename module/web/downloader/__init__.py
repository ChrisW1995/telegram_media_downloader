# -*- coding: utf-8 -*-
"""Message Downloader main module (Quart async)

Handles root path (/) and login page routes
"""
...
@bp.route("/")
async def index():
    """Message Downloader main page (Quart async) - now at root path"""
    # Check authentication status
    if not session.get('authenticated', False):
        # Store original request URL for redirect after login
        session['next_url'] = request.url
        return redirect(url_for('message_downloader.login'))
...
@bp.route("/login")
async def login():
    """Message Downloader login page (Quart async)"""
    # If already logged in, redirect to target page
    if session.get('authenticated', False):
        next_url = session.pop('next_url', url_for('message_downloader.index'))
...
def register_blueprints(flask_app, original_app=None):
    ...
    from . import auth, groups, downloads, thumbnails, video_frames, video_stream
    ...
    if original_app:
        ...
        video_frames.set_app_instance(original_app)
        video_stream.set_app_instance(original_app)
