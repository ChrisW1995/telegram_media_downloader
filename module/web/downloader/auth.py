"""Message Downloader 認證 API 模組 (Quart async)
...
async def send_code():
    ...
    try:
        ...
        # Quart: 直接 await async 函數
        result = await auth_manager.start_auth_process(phone_number, api_id, api_hash)
...
async def verify_code():
    ...
    try:
        ...
        # Quart: 直接 await async 函數
        result = await auth_manager.verify_code(session_key, verification_code, phone_code_hash)
...
async def verify_password():
    ...
    try:
        ...
        # Quart: 直接 await async 函數
        result = await auth_manager.verify_password(session_key, password)
...
async def force_clear_session():
    ...
    try:
        ...
        # Return response (Quart handles cookies differently)
        from quart import make_response
        response = await make_response(success_response(message="所有 session 數據已強制清除"))

        # Clear any possible cookies
        response.set_cookie('session', '', max_age=0, httponly=True, path='/')
...
async def qr_login():
    ...
    try:
        ...
        # Quart: 直接 await async 函數
        result = await auth_manager.start_qr_login(api_id, api_hash)
...
async def check_qr_status():
    ...
    try:
        ...
        # Quart: 直接 await async 函數
        result = await auth_manager.check_qr_status(session_key)
